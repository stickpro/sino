...
function syn($text = null) {
    ...
    foreach ($syn as $one) {
        $ww = explode('|', trim($one));
        $vars = explode(', ', $ww[1]);
        $word = strtolower($ww[0]);
        $swords[$word] = $vars;
        $cvars = array();
        foreach ($vars as $ok) {
            $cvars[] = ucfirst(strtolower(trim($ok)));
        }
        $swords[ucfirst($word)] = $cvars;
    }
    $callback = function ($matches)use ($swords)
    {
        return isset($swords[$matches[1]]) ? $swords[$matches[1]][array_rand($swords[$matches[1]])] :
            $matches[1];
    };
    echo '<pre>';
    var_dump($callback);
